...
import { FrontendApi, Configuration, RegistrationFlow } from '@ory/client'
...
export default function RegistrationPage() {
  ...
  useEffect(() => {
    if (flowId) {
      ory.getRegistrationFlow({ id: flowId }).then(({ data }) => setFlow(data)).catch(() => router.push('/auth/registration'))
    } else {
      ory.createBrowserRegistrationFlow().then(({ data }) => router.push(`/auth/registration?flow=${data.id}`))
    }
  }, [flowId, router])
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await ory.updateRegistrationFlow({ flow: flow.id, updateRegistrationFlowBody: { method: 'password', password: formData.get('password') as string, traits: { email: formData.get('email') as string } } })
      router.push('/')
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center">
      <form onSubmit={handleSubmit} className="w-full max-w-md p-8 bg-white shadow rounded">
        ...
        <a href="/auth/login" className="block mt-4 text-center text-blue-600">Already have an account?</a>
      </form>
    </div>
  )
}
