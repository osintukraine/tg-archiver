...
# Import BoxSelector for dynamic storage box selection
...
class MediaArchiver:
    ...
    def __init__(
        ...
        Args:
            ...
            storage_box_id: Fixed storage box ID (legacy). If set, disables dynamic selection.
            box_selector: BoxSelector for dynamic box selection. Created if not provided.
            account_region: Region for box selection (e.g., "russia", "ukraine")
        """
        self.minio = minio_client
        self.redis = redis_client
        self.storage_box_id = storage_box_id
        self.box_selector = box_selector or BoxSelector()
    ...
    async def _select_storage_box(
        self,
        session: AsyncSession,
        file_size: int,
    ) -> str:
        """
        Select storage box for a new file.

        Uses BoxSelector for dynamic selection if no fixed box_id is set.
        Falls back to "default" if selection fails.

        Args:
            session: Database session for BoxSelector queries
            file_size: Size of file in bytes (for capacity reservation)

        Returns:
            Storage box ID string
        """
        # Use fixed box if configured (legacy behavior)
        if self.storage_box_id:
            return self.storage_box_id

        # Dynamic selection via BoxSelector
        try:
            box = await self.box_selector.get_box_for_file(
                session,
                file_size,
            ...
            if box:
                logger.debug(f"Selected storage box: {box.id} for {file_size} bytes")
                return box.id
        except Exception as e:
            logger.error(f"BoxSelector failed: {e}, falling back to 'default'")

        # Fallback
        logger.warning("No eligible storage box, using 'default'")
        return "default"
    ...
    async def archive_media(
        ...
        try:
            ...
            if existing_file:
                ...
                selected_box_id = await self._select_storage_box(session, file_size)
